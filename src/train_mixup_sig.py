...
def train_loop(
    ...
    while not manager.stop_trigger:
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = sigmoid_cancat_mix(data, target)
...
def sample_mask(size, sigmoid_range=(3, 12), use_cuda=True):
    x_radius = random.randint(*sigmoid_range)
...
def sigmoid_cancat_mix(x, y, use_cuda=True):
    mix_mask = sample_mask(x.shape[-2:])
    ...
    mixed_x = mix_mask * x + rnd_mix_mask * rnd_image
    mixed_y = y + rnd_target
    mixed_y = np.clip(mixed_y, 0.0, 1.0)
    return mixed_x, mixed_y
