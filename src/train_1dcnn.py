...
def train(config_path, short_mode):
    ...
    print('build_model')
    model = get_conv_model(settings["model"])
    model = model.to(device)
    print('set_train_conf...')
    # get optimizer
    optimizer = getattr(
...
def standardize(X: np.ndarray, mean=None, std=None, norm_max=None, norm_min=None, eps=1e-6):
    mean = mean or X.mean()
    X = X - mean
    std = std or X.std()
    Xstd = X / (std + eps)
...
def get_conv_model(args: tp.Dict):
    model = CnnModel(
        in_channels=1,
        n_cont_features=10,  # 適当
        hidden_channels=args["params"]["hidden_channels"],
        kernel_sizes=args["params"]["kernel_sizes"],
        out_dim=args["params"]["n_classes"]
    )
    return model
